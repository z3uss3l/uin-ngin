# Apply UI patch, run tests and push
# Usage: Open PowerShell in repo root and run: .\scripts\apply_uin_ui_patch.ps1
set -e
...
$files = @(
  "packages/uin-ui/App.jsx",
  "packages/uin-ui/src/App.test.jsx",
  "packages/uin-ui/package.json"
)
foreach ($f in $files) {
  if (Test-Path $f) {
    Copy-Item $f "$f.bak" -Force
    Write-Host "Backed up $f -> $f.bak"
  }
}

Write-Host "Writing updated App.jsx..."
$AppContent = @'
...
'@
Set-Content -Path "packages/uin-ui/App.jsx" -Value $AppContent -Encoding UTF8
...
New-Item -ItemType Directory -Force -Path "packages/uin-ui/src" | Out-Null
Set-Content -Path "packages/uin-ui/src/App.test.jsx" -Value $TestContent -Encoding UTF8

# Update package.json to ensure test script & devDependencies
$pkgPath = "packages/uin-ui/package.json"
if (Test-Path $pkgPath) {
  $pkg = Get-Content $pkgPath -Raw | ConvertFrom-Json
  $pkg.scripts.test = "react-scripts test --env=jsdom --watchAll=false --runInBand"
  ...
  Write-Host "package.json not found at $pkgPath"; exit 1
}

# Run npm install, tests and build
Push-Location "packages/uin-ui"
try {
  ...
  Write-Host "One of the npm steps failed. Restoring backups..."; exit 1
} finally {
  Pop-Location
}

# Git commit & push (robust when branch already exists)
try {
  Write-Host "Committing and pushing changes..."
  $branch = 'feature/ui-polish-tests'
  $branchExists = (& git branch --list $branch).Trim()
  if ($branchExists) {
    Write-Host "Branch $branch exists locally — switching to it"
    git switch $branch
  } else {
  ...
  Write-Host "Git operations failed: $_"; # don't exit hard - report to user
}
