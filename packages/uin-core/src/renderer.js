...
/**
 * Render group of humans
 */
function renderHumanGroup(obj, p, scale, opacity) {
  const h = (obj.measurements?.height?.value || 1.7) * scale * 80;
  const radius = h * 0.15;
  const positions = [
    { x: 0, y: 0 },
    { x: -radius * 1.5, y: radius * 0.5 },
    { x: radius * 1.5, y: radius * 0.5 }
  ];

  const elements = [];

  // Render distinct circular stick figures for group
  const groupColor = obj.features?.clothing?.color || '#0891b2';

  positions.forEach(pos => {
    // Head
    elements.push(`<circle cx="${p.x + pos.x}" cy="${p.y - h + pos.y}" r="${radius}" fill="#FFD7B5" opacity="${opacity}"/>`);
    // Body
    elements.push(`<rect x="${p.x + pos.x - radius}" y="${p.y - h + pos.y + radius}" width="${radius * 2}" height="${h * 0.6}" fill="${groupColor}" opacity="${opacity}"/>`);
  });
